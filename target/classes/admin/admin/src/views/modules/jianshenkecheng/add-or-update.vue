	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="上课时间" prop="shangkeshijian">
					<el-date-picker
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.shangkeshijian" label="上课时间" prop="shangkeshijian">
                ...
                <el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="课程层级" prop="gonghao">
                    <el-select :disabled="ro.gonghao" @change="gonghaoChange" v-model="ruleForm.curriculumlevel" placeholder="请选择课程层级">
                        <el-option
                ...
                <el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.curriculumlevel" label="课程层级" prop="gonghao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.kechengjieshao" label="课程介绍" prop="kechengjieshao">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.kechengjieshao"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
                ...
                curriculumlevel: [
					{ required: true, message: '课程层级不能为空', trigger: 'blur' },
				],
                gonghao: [
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				url: `option/jianshenjiaolian/gonghao`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
			...
			gonghaoChange () {
				this.$http({
					url: `follow/jianshenjiaolian/gonghao?columnValue=`+ this.ruleForm.gonghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
