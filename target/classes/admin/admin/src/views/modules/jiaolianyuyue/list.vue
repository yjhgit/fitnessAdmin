<template>
	<div class="main-content">
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"padding":"10px","margin":"20px 0","background":"#95d5f2","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(57,148,220,1) 0%, rgba(34,114,194,1) 100%)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('jiaolianyuyue','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(220,57,57,1) 0%, rgba(194,34,34,1) 100%)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('jiaolianyuyue','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>




					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(57,220,217,1) 0%, rgba(34,194,182,1) 100%)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('jiaolianyuyue','预约人数统计')" type="warning" @click="chartDialog1()">预约人数统计</el-button>
				</el-row>
			</el-form>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="jiaolianxingmingChart1" style="width:100%;height:600px;"></div>
...
export default {
  ...
  methods: {
    ...
    yuyuequxiaoCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      ...
      this.$storage.set('crossTable','jiaolianyuyue');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        var jiaolianxingmingChart1 = echarts.init(document.getElementById("jiaolianxingmingChart1"),'vintage');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                jiaolianxingmingChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    jiaolianxingmingChart1.resize();
                };
            }
        });
      })
