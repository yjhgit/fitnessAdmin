...
<div :style='{"width":"1000px","padding":"20px","margin":"10px auto","position":"relative","background":"#D8D8D8"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 2% 10px","display":"inline-block"}' label="申请时间" prop="shenqingshijian" disabled>
              <el-date-picker
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 2% 10px","display":"inline-block"}' label="申请说明" prop="shenqingshuoming">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="申请说明"
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/jianshenkecheng/kechengmingcheng', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      kechengmingchengChange () {
        this.$http.get('follow/jianshenkecheng/kechengmingcheng?columnValue=' + this.ruleForm.kechengmingcheng, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('diaokeshenqing/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('diaokeshenqing/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('diaokeshenqing/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
