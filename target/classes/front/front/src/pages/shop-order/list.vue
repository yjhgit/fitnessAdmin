                        ...
                        if(item.type==1||item.type==3) {
                            ...
                            this.$http.post(table+'/update', this.user).then(res => {
                                ...
                                this.$http.post(`orders/update`, item).then(res => {
                                    if(item.tablename == `jianshenqicai`){
                                        // 加回库存数量
                                        good.alllimittimes = good.alllimittimes + item.buynumber;
                                        this.$http.post(`jianshenqicai/update`, good).then(res => {
                                        });
                                    }
                                    if(item.tablename == `jianshenshangpin`){
                                        // 加回库存数量
                                        good.alllimittimes = good.alllimittimes + item.buynumber;
                                        this.$http.post(`jianshenshangpin/update`, good).then(res => {
                                        });
                                    }
                                    this.$message({
                                      message: '退款成功',
